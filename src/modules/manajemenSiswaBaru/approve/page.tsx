...
import pageData from "../approve/approve.json";
import "./css/page.css";
...
function Approve() {
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Siswa Approve");
    XLSX.writeFile(wb, "siswa-baru-approve.xlsx");
    ...
    reader.onload = (event) => {
      try {
        ...
        const imported: Siswa[] = jsonData.map((row, index) => ({
          ...
          status: row["Status"] || row["status"] || "Approve",
        }));
