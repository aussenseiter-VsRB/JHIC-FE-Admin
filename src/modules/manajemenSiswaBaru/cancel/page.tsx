...
import pageData from "../cancel/cancel.json";
import "./css/page.css";
...
function Cancel() {
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Siswa Cancel");
    XLSX.writeFile(wb, "siswa-baru-cancel.xlsx");
    ...
    reader.onload = (event) => {
      try {
        ...
        const imported: Siswa[] = jsonData.map((row, index) => ({
          ...
          status: row["Status"] || row["status"] || "Cancel",
        }));
